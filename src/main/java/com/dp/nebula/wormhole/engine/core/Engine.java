...
public class Engine {
	...
	public int run(JobConf jobConf){
		...
		try{
			...
			AbstractPluginManager.regDataSourceProp(readerConf.getPluginParam());
			//get source and target info before prepare phase, due to it may throw exception during prepare
			source = readerConf.getPluginName() + "/" + readerConf.getPluginParam().getValue(IP, "IP_UNKNOWN")
			...
			for(int i =0 ; i < writerConf.size(); i++){
				...
				sb.append(conf.getPluginName()).append("/").append(conf.getPluginParam().getValue(IP, "IPUnknown"))
					.append("/").append(conf.getPluginParam().getValue(DB, ""))
					.append("/").append(conf.getPluginParam().getValue(TABLE, ""));
			...
			IParam readerPluginParam = pluginReg.get(readerConf.getPluginName());
			...
			int intervalCount = 0;
			int statusCheckInterval = engineConf.getIntValue(EngineConfParamKey.STATUS_CHECK_INTERVAL, STATUS_CHECK_INTERVAL);
			int monitorInfoDisplayPeriod = engineConf.getIntValue(EngineConfParamKey.MONITOR_INFO_DISPLAY_PERIOD, INFO_SHOW_PERIOD);
			writerConsistency = engineConf.getBooleanValue(EngineConfParamKey.WRITER_CONSISTENCY, false);

			while(true){
				intervalCount ++;
				statusCode = checkStatus(readerManager, writerManager, monitorManager, storageManager);
				status = JobStatus.fromStatus(statusCode);
				if(status == null) {
					s_logger.error("status = " + statusCode + ".This should never happen");
					return JobStatus.FAILED.getStatus();
				}
				//read failed rollback all
				if(status == JobStatus.FAILED || (status.getStatus()>=JobStatus.FAILED.getStatus()&&status.getStatus()<JobStatus.WRITE_FAILED.getStatus())){
					s_logger.error("Nebula wormhole Job is Failed!");
					writerManager.rollbackAll();
					break;
				}
				else if(status == JobStatus.PARTIAL_FAILED || status.getStatus()>=JobStatus.WRITE_FAILED.getStatus()){
					Set<String> failedIDs = getFailedWriterIDs(writerManager, monitorManager);
					s_logger.error("Some of the writer is Failed:" + failedIDs.toString());
				...
				//Running
					...
					try {
						...
						s_logger.error("Sleep of main thread is interrupted!", e);
					}
				}
			}
			...
			if(JobStatus.fromStatus(e.getStatusCode()).equals(JobStatus.ROLL_BACK_FAILED)) {
			...
			if(monitorManager != null) {
				WormHoleJobInfo jobInfo = monitorManager.getJobInfo(source,target,time/1000,status.getStatus(),now);
	 ...
	 * @return
	 */
	private int checkStatus(ReaderManager readerManager, WriterManager writerManager,
			MonitorManager monitorManager, StorageManager storageManager){
		...
		if(readerTerminated && writerTerminated && readerSuccess &&
			...
			for(FailedInfo fi: failedInfoSet){
				...
				if(size >= 0) {
					s_logger.info(writerID + ": " + fi.getFailedLines() + " lines failed to write" );
				} else {
					s_logger.info(writerID + ": " + (-fi.getFailedLines()) + " lines duplicated to write" );
				}
				if(Math.abs(fi.getFailedLines()) >= writerManager.getFailedLinesThreshold(writerID)) {
		...
		else if(readerTerminated && readerSuccess && writerTerminated && failedWriterIDs.size() > 0){
	...
	private Set<String> getFailedWriterIDs(WriterManager writerManager, 
		...
		for(FailedInfo fi: failedInfoSet){
			String writerID = fi.getFailedWriterID();
			if(fi.getFailedLines() >= writerManager.getFailedLinesThreshold(writerID)){
				r.add(writerID);
			}
		}
	...
	private List<StorageConf> createStorageConfs(JobConf jobConf){
		...
		for(JobPluginConf jobPluginConf: writerConfList){
			...
			storageConf.setStorageClassName(
					engineConf.getValue(EngineConfParamKey.STORAGE_CLASS_NAME, DEFAULT_STORAGE_CLASS_NAME));
			storageConf.setLineLimit(
					engineConf.getIntValue(EngineConfParamKey.STORAGE_LINE_LIMIT, DEFAULT_STORAGE_LINE_LIMIT));
			storageConf.setByteLimit(
					engineConf.getIntValue(EngineConfParamKey.STORAGE_BYTE_LIMIT, DEFAULT_STORAGE_BYTE_LIMIT));
			storageConf.setDestructLimit(
					engineConf.getIntValue(EngineConfParamKey.STORAGE_DESTRUCT_LIMIT, DEFAULT_STORAGE_DESTRUCT_LIMIT));
			storageConf.setPeriod(
					engineConf.getIntValue(EngineConfParamKey.MONITOR_INFO_DISPLAY_PERIOD,INFO_SHOW_PERIOD));
			storageConf.setWaitTime(
					engineConf.getIntValue(EngineConfParamKey.STORAGE_WAIT_TIME,DEFAULT_STORAGE_WAIT_TIME));
