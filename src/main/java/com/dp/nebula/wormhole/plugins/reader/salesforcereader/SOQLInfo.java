...
class SOQLInfo {
	...
	static class SOQLFieldInfo {
		...
		SOQLFieldInfo(String fieldString, AtomicInteger aggregateFieldIdx)
				throws SOQLParserException {
			fieldString = getTrimmed(fieldString);
			int lparenIdx = fieldString.indexOf('(');
			// no nested queries!
			if (lparenIdx == 0)
				throw invalidSoql("Nested queries are not supported");
			if (lparenIdx < 0) {
				// normal field
				...
				if (rparenIdx < 0)
					throw invalidSoql("Could not find closing paren");
				this.aggregateFunction = fieldString.substring(0, lparenIdx);
				this.fieldName = getTrimmed(fieldString.substring(
						lparenIdx + 1, rparenIdx));
				this.alias = determineAggregateAlias(aggregateFieldIdx,
						fieldString.substring(rparenIdx + 1));
			}
		}
		...
		private String determineAggregateAlias(AtomicInteger aggregateFieldIdx,
				String aliasString) {
			String alias = aliasString.trim();
			if (alias.length() == 0) {
				alias = aggregateFieldIdx == null ? null : "expr"
						+ aggregateFieldIdx.getAndIncrement();
			}
			return alias;
		}
		...
		public String toString() {
			if (isAggregate())
				return getAggregateFunction() + "(" + getFieldName() + ")";
			return getFieldName();
		}
	...
	SOQLInfo(String soql) throws SOQLParserException {
		...
		if (!soqlLower.startsWith(SELECT_KEYWORD))
			throw invalidSoql("No 'SELECT' keyword");
		...
		String rawFields = soql.substring(SELECT_KEYWORD.length(), fromIdx)
				.trim();
		...
		String remainder = getTrimmed(soql.substring(
				fromIdx + FROM_KEYWORD.length()).trim());
		...
		if (stok.hasMoreTokens()) {
			...
			if (WHERE_KEYWORD.equalsIgnoreCase(alias))
				alias = "";
		}
