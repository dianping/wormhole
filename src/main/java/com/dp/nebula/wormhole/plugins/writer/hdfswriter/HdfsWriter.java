...
public class HdfsWriter extends AbstractPlugin implements IWriter {
	...
	/*
	 * fieldSplit,lineSplit …… 基本参数的设定
	 * 获取 HDFS 读写的handler————fs
	 * */
	@Override
	public void init() {
	...
	class DfsWriterTextFileStrategy implements DfsWriterStrategy {
		...
		/*
		 * 从传入的参数 receiver 读取数据， 然后根据需要 (transformer) 将读取的行转换为自己需要的格式
		 * 然后 通过 BufferedWriter 写入到 HDFS
		 * */
		@Override
