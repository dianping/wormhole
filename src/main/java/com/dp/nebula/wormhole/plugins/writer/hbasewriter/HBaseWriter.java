...
public class HBaseWriter extends AbstractPlugin implements IWriter {
	...
	//private boolean putWithTs;
	//private long decHours;
	...
	public void init() {
		...
		//putWithTs   = getParam().getBooleanValue(ParamKey.putWithTs);
		//decHours    = getParam().getLongValue(ParamKey.hoursDecTimeStamp);
	...
	public void write(ILineReceiver lineReceiver) {
		...
		try {
			while ((line = lineReceiver.receive()) != null) {
				...
				//modifyed by mt, if putWithTs is true , use setRowKeyWithTs
				/*if ( putWithTs ){
					client.setRowKeyWithTs(rowKey.getBytes(DEFAULT_ENCODING), putTimeStamp);
				} else {
