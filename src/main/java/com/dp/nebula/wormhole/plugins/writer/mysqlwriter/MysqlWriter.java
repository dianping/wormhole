...
public class MysqlWriter extends AbstractPlugin implements IWriter {
	...
	public void init() {
		...
		this.replace = getParam().getBooleanValue(ParamKey.REPLACE, false);
	...
	public void connection() {
		...
		try {
			conn = DBSource.getConnection(this.getClass(), ip, writerID, dbname);
		} catch (Exception e) {
	...
	public void write(ILineReceiver receiver) {
		...
		try {
			...
			this.logger.debug(String.format(
					...
					MAX_ERROR_COUNT));
			DBUtils.update(conn,String.format("set max_error_count=%d;",
					MAX_ERROR_COUNT));
			...
			while((line = receiver.receive()) != null){
				...
				if(getMonitor().getFailedLines()>failedLinesThreshold) {
					logger.error(writerID + " Mysqlwriter :Failed lines threshold is larger than expect");
					throw new WormholeException("Mysqlwriter :Failed lines threshold is larger than expect",JobStatus.WRITE_FAILED.getStatus(),writerID);
				}
			...
			if (null != this.conn) {
				try {
					...
					throw new WormholeException(e1,JobStatus.WRITE_FAILED.getStatus() + errorCodeAdd,writerID);
				}
			}
		}
	}
	...
	private void makeLoadSql(){
		if(replace){
			sql = String.format(REPLACE_SQL_PATTERN, table, splitColumns(columns));
		}
		sql = String.format(INSERT_SQL_PATTERN, table, splitColumns(columns));
	}
	...
	/**
	 * @param der column list from job configure
